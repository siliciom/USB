  ...
  task token_packet_received(USB2p0_sequence_item usb_seq_item,bit [DATA_WIDTH-1:0]device_dri_rxdata[$]);
          ...
          `uvm_info("TOKEN_PKT",$sformatf("RECEIVED_OUT_TOKEN_PKT - TOKEN: PID=%0h ADDR=%0d ENDP=%0d CRC_5=%0d",token_pid_setup, token_addr, token_endp,token_crc5),UVM_LOW)
           if(token_endp == control_transfer_endp)begin
            ...
            `uvm_info("DEVICE_DRIVER","RECEIVED_INTERRUPT_OUT_ENDP",UVM_LOW)
               receive_host_payload(data_received);
           end
     ...
     begin
       ...
       if(pid_byte[7:4] == PID_ACK)
          `uvm_info("HANSHAKE_PKT",$sformatf("RECEIVED_HS_ACK_PID=%0h,",pid_byte),UVM_LOW)
       if(pid_byte[7:4] == PID_NAK)
          `uvm_info("HANSHAKE_PKT",$sformatf("RECEIVED_HS_NACK_PID=%0h,",pid_byte),UVM_LOW)
       if(pid_byte[7:4] == PID_STALL)
          `uvm_info("HANSHAKE_PKT",$sformatf("RECEIVED_HS_STALL_PID=%0h",pid_byte),UVM_LOW)
      end
...
////////////////////////////////////////////////////////////////
             ...
             valid_request = 0;
            // GET_STATUS
            if(bmRequest == 8'd128 && bRequest == 8'd0)
               valid_request = 1;
 
            // CLEAR_FEATURE
            else if(bmRequest == 8'd0 && bRequest == 8'd1)
               valid_request = 1;
 
            // SET_FEATURE
            else if(bmRequest == 8'd0 && bRequest == 8'd3)
               valid_request = 1;
 
            // SET_ADDRESS
            else if(bmRequest == 8'd0 && bRequest == 8'd5)
               valid_request = 1;
 
            // GET_DESCRIPTOR
            else if(bmRequest == 8'd128 && bRequest == 8'd6)
               valid_request = 1;
 
            // GET_CONFIGURATION
            else if(bmRequest == 8'd128 && bRequest == 8'd8)
               valid_request = 1;
 
            // SET_CONFIGURATION
            else if(bmRequest == 8'd0 && bRequest == 8'd9)
               valid_request = 1;
 
            // GET_INTERFACE
            else if(bmRequest == 8'd129 && bRequest == 8'd10)
               valid_request = 1;
 
            // SET_INTERFACE
            else if(bmRequest == 8'd1 && bRequest == 8'd11)
               valid_request = 1;
            ...
            if(valid_request) begin
               ...
               `uvm_error("SETUP", $sformatf("INVALID_REQUEST bmRequest=%0h bRequest=%0h", bmRequest,bRequest))
               send_handshake_packet(PID_STALL,req);
               return;
            end
  ...
  task received_zlp_data(bit [DATA_WIDTH-1:0] data_received[$]);
   ...
   endtask
	...
	`uvm_info("DEVICE_DRIVER",$sformatf("handshake_pid=%0h",pid_byte),UVM_LOW);
	 tx_data.push_back(pid_byte);
	 `uvm_info("DEVICE_DRIVER",$sformatf("handshake_tx_pid_byte %0p",tx_data),UVM_LOW);
	 send_to_utmi(tx_data,req);
	 `uvm_info("DEVICE_CONTROLLER_DRIVER",$sformatf("COMPLETED_SEND_HANDSHAKE_PACKET"),UVM_LOW);
    endtask*/
        ...
        pid_byte = {handshake_pid, ~handshake_pid};
         ...
         if (usb_seq_item.word_if) begin
             `uvm_info("DEVICE_DRIVER",$sformatf("HANDSHAKE_PID_SENT_FOR_WORD_IF_ONE"), UVM_LOW)
              ...
              // if (i == 0)
                //utmi_interface_tx.utmi_txdata[i*8 +: 8] <= pid_byte;
               //else
             //   utmi_interface_tx.utmi_txdata[i*8 +: 8] <= 8'h00;
           //end      
...
endclass
       ...
       //get_congfiguration
       //   if(bmRequest == 8'd128 && bRequest == 8'd8)begin
         //    send_get_configuration(usb_seq_item);
          // end
 /*          //get_interface
          if(bmRequest == 8'd1 && bRequest ==8'd10)begin         
             send_get_interface(usb_seq_item);
          end
          //get_status
          if( (bmRequest == 8'd128 ||bmRequest == 8'd129 ||bmRequest == 8'd130) && bRequest == 8'd0)begin 
             send_get_status(usb_seq_item);
         end
        //get_descriptor 
         if(bmRequest == 8'd128 && bRequest ==8'd6)begin 
              send_device_descriptor(usb_seq_item);
         end*/
