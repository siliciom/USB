class usb2p0_host_hs_isochronous_out_sequence extends USB2p0_HOST_sequence;
  
	`uvm_object_utils(usb2p0_host_hs_isochronous_out_sequence)
  	USB2p0_sequence_item req;

       function new(string name="usb2p0_host_hs_isochronous_out_sequence");
            super.new(name);
       endfunction

    task body();
           `uvm_info("HS_ISO_SEQUENCE","ENTERED_INTO_HS_ISOCHRONOUS_SEQ ",UVM_LOW)
		...
		`uvm_fatal("RAND_FAIL", "HOST_LS_data_stage_seq_Randomization failed")
          ...
          finish_item(req);
	  req.print(); 
           `uvm_info("HS_ISO","COMPLETED_HS_ISOCHRONOUS_SEQ ",UVM_LOW)
