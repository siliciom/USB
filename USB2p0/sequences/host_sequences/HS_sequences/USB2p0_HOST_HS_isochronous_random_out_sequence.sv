...
class usb2p0_host_hs_isochronous_random_out_sequence extends USB2p0_HOST_sequence;
  
	`uvm_object_utils(usb2p0_host_hs_isochronous_random_out_sequence)
  	USB2p0_sequence_item req;

       function new(string name="usb2p0_host_hs_isochronous_random_out_sequence");
            super.new(name);
       endfunction
    ...
    task body();
       ...
       `uvm_info(get_full_name(),"ENTERED_INTO_RANDOM_CONTROL_SEQUENCE",UVM_LOW)
        ...
        if(!req.randomize() with {
                                  transfer_type == ISO_CHRONOUS_TRANSFER;
				  otg_vbusvalid ==  1'b1;
                                  addr             ==  7'd0;
                                  iso_dir     ==   0;
				  direction         ==   0;
