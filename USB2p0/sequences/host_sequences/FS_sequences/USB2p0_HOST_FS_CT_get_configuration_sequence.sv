class usb2p0_host_fs_ct_get_configuration_sequence extends USB2p0_HOST_sequence;
  
	`uvm_object_utils(usb2p0_host_fs_ct_get_configuration_sequence)
  	USB2p0_sequence_item req;

       function new(string name="usb2p0_host_fs_ct_get_configuration_sequence");
            super.new(name);
       endfunction

    task body();
           `uvm_info("FS_GET_CONFIGURATION","ENTERED_INTO_FS_GET_CONFIGURATION_SEQ ",UVM_LOW)
		...
		`uvm_fatal("RAND_FAIL", "HOST_LS_data_stage_seq_Randomization failed")
          ...
          finish_item(req);
	  req.print(); 
           `uvm_info("FS_GET_CONFIGURATION","COMPLETED_FS_GET_CONFIGURATION_SEQ ",UVM_LOW)
