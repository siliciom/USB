class usb2p0_host_fs_bulk_in_sequence extends USB2p0_HOST_sequence;
  
	`uvm_object_utils(usb2p0_host_fs_bulk_in_sequence)
  	USB2p0_sequence_item req;

       function new(string name="usb2p0_host_fs_bulk_in_sequence");
            super.new(name);
       endfunction
    ...
    task body();
	    ...
	    if(!req.randomize() with {transfer_type == BULK_TRANSFER;
	                             ...
	                             // otg_dmpulldown   ==  1;
                                      otg_vbusvalid    ==  1'b1;
                                      bulk_in_pid ==   USB2p0_sequence_item::PID_IN;
                                      addr             ==  7'd0;
                                      endp             ==  4'd1;
                                      rx_valid         ==  'd0;
	         		      rx_validh        ==  'd0; }) begin
