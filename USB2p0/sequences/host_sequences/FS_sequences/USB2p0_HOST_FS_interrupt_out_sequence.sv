class usb2p0_host_fs_interrupt_out_sequence extends USB2p0_HOST_sequence;
  
	`uvm_object_utils(usb2p0_host_fs_interrupt_out_sequence)
  	USB2p0_sequence_item req;

       function new(string name="usb2p0_host_fs_interrupt_out_sequence");
            super.new(name);
       endfunction
    ...
    task body();
		...
		`uvm_fatal("RAND_FAIL", "HOST_LS_data_stage_seq_Randomization failed")
          ...
          finish_item(req);
	  req.print(); 
           `uvm_info("FS_INTERRUPT_SEQUENCE","COMPLETED_FS_INTERRUPT_SEQ ",UVM_LOW)
