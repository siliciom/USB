class usb2p0_ls_interrupt_in_vsequence extends usb2p0_base_virtual_sequence;
    `uvm_object_utils(usb2p0_ls_interrupt_in_vsequence)
    ...
    usb2p0_host_ls_interrupt_in_sequence      interrupt_in_seq;

  function new (string name = "usb2p0_ls_interrupt_in_vsequence");
    super.new(name);
  endfunction
...
task usb2p0_ls_interrupt_in_vsequence::body();

     `uvm_info("LS_CONTROL_TRANSFER_VSEQ","ENTERED_INTO_INTERRUPT_IN_VSEQ ",UVM_LOW)
  ...
  //  phy_data_stage_seq=USB2p0_phy_LS_data_stage_sequence::type_id::create("phy_data_stage_seq");
    ...
    interrupt_in_seq=usb2p0_host_ls_interrupt_in_sequence::type_id::create("interrupt_in_seq");
...
//
	  ...
	  device_handshake_send.start(p_sequencer.device_seqr); 
     join 
     `uvm_info("LS_CONTROL_TRANSFER_VSEQ","COMPLETED_INTO_INTERRUPT_IN_TRANSFER_VSEQ ",UVM_LOW)
endtask
