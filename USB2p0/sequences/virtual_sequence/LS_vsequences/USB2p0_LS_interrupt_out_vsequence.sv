class usb2p0_ls_interrupt_out_vsequence extends usb2p0_base_virtual_sequence;
    `uvm_object_utils(usb2p0_ls_interrupt_out_vsequence)
    ...
    usb2p0_host_ls_interrupt_out_sequence      interrupt_out_seq;

  function new (string name = "usb2p0_ls_interrupt_out_vsequence");
    super.new(name);
  endfunction
...
task usb2p0_ls_interrupt_out_vsequence::body();

     `uvm_info("LS_CONTROL_TRANSFER_VSEQ","ENTERED_INTO_INTERRUPT_OUT_VSEQ ",UVM_LOW)
    ...
    interrupt_out_seq=usb2p0_host_ls_interrupt_out_sequence::type_id::create("interrupt_out_seq");
...
//
	  ...
	  //device_handshake_send.start(p_sequencer.device_seqr); 
     join 
     `uvm_info("LS_CONTROL_TRANSFER_VSEQ","COMPLETED_INTO_INTERRUPT_OUT_TRANSFER_VSEQ ",UVM_LOW)
endtask
