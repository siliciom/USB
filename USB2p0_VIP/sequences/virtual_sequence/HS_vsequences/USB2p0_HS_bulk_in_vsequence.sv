class usb2p0_hs_bulk_in_vsequence extends usb2p0_base_virtual_sequence;
    `uvm_object_utils(usb2p0_hs_bulk_in_vsequence)
    ...
    usb2p0_host_hs_bulk_in_sequence                  bulk_in_seq;

  function new (string name = "usb2p0_hs_bulk_in_vsequence");
    super.new(name);
  endfunction
...
task usb2p0_hs_bulk_in_vsequence::body();

     `uvm_info("HS_CONTROL_TRANSFER_VSEQ","ENTERED_INTO_BULK_IN_VSEQ ",UVM_LOW)
    ...
    bulk_in_seq=usb2p0_host_hs_bulk_in_sequence::type_id::create("bulk_in_seq");
...
//
	  ...
	  device_handshake_send.start(p_sequencer.device_seqr); 
     join 
     `uvm_info("HS_CONTROL_TRANSFER_VSEQ","COMPLETED_INTO_BULK_IN_TRANSFER_VSEQ ",UVM_LOW)
endtask
