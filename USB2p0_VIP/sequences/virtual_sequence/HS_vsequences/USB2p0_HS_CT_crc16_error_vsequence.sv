class usb2p0_hs_ct_crc16_error_vsequence extends usb2p0_base_virtual_sequence;
    `uvm_object_utils(usb2p0_hs_ct_crc16_error_vsequence)
    ...
    usb2p0_host_hs_ct_crc16_error_sequence    status_in_token_seq;

  function new (string name = "usb2p0_hs_ct_crc16_error_vsequence");
    super.new(name);
  endfunction
...
task usb2p0_hs_ct_crc16_error_vsequence::body();

     `uvm_info("HS_CONTROL_TRANSFER_VSEQ","ENTERED_INTO_HS_CRC16_ERROR_VSEQ ",UVM_LOW)
    ...
    status_in_token_seq=usb2p0_host_hs_ct_crc16_error_sequence::type_id::create("status_in_token_seq");
	...
	device_handshake_send.start(p_sequencer.device_seqr); 
     join 
     `uvm_info("HS_CONTROL_TRANSFER_VSEQ","COMPLETED_INTO_HS_CRC16_ERROR_VSEQ ",UVM_LOW)
endtask
