class usb2p0_fs_ct_set_feature_vsequence extends usb2p0_base_virtual_sequence;
    `uvm_object_utils(usb2p0_fs_ct_set_feature_vsequence)
    ...
    usb2p0_host_fs_ct_set_feature_in_sequence  status_in_token_seq;

  function new (string name = "usb2p0_fs_ct_set_feature_vsequence");
    super.new(name);
  endfunction
...
task usb2p0_fs_ct_set_feature_vsequence::body();

     `uvm_info("FS_CONTROL_TRANSFER_VSEQ","ENTERED_INTO_FS_CONTROL_TRANSFER_SET_FEATURE_VSEQ ",UVM_LOW)
    ...
    status_in_token_seq=usb2p0_host_fs_ct_set_feature_in_sequence::type_id::create("status_in_token_seq");
...
//
	...
	device_handshake_send.start(p_sequencer.device_seqr); 
     join 
     `uvm_info("FS_CONTROL_TRANSFER_VSEQ","COMPLETED_INTO_FS_CONTROL_TRANSFER_SET_FEATURE_VSEQ ",UVM_LOW)
endtask
