class USB2p0_DEVICE_hs_interrupt_in_error_sequence extends USB2p0_DEVICE_sequence;

  `uvm_object_utils(USB2p0_DEVICE_hs_interrupt_in_error_sequence)
   USB2p0_sequence_item     req;

     function new(string name="USB2p0_DEVICE_hs_interrupt_in_error_sequence");
        super.new(name);
     endfunction
 
    task body();
        `uvm_info("HS_CONTROL_TRANSFER_SEQ","STARTED_DEVICE_HS_INTERRUPT_TRASFER_ERROR_SEQ ",UVM_LOW)
	...
	if(!req.randomize() with { otg_dmpulldown == 0;
         ...
         start_item(req);
         finish_item(req);
         `uvm_info("HS_INTERRUPT_TRANSFER_SEQ","COMPLETED_DEVICE_HS_INTERRUPT_TRANSFER__ERROR_SEQ",UVM_LOW)
    endtask
