class USB2p0_DEVICE_hs_send_hs_sequence extends USB2p0_DEVICE_sequence;

  `uvm_object_utils(USB2p0_DEVICE_hs_send_hs_sequence)
   USB2p0_sequence_item     req;

     function new(string name="USB2p0_DEVICE_hs_send_hs_sequence");
        super.new(name);
     endfunction
 
    task body();
        `uvm_info("HS_CONTROL_TRANSFER_SEQ","STARTED_DEVICE_HS_CONTROL_TRANSFER_HS_SEQ ",UVM_LOW)
	...
	if(!req.randomize() with { op_mode             == 2'b00;
         ...
         start_item(req);
         finish_item(req);
         `uvm_info("HS_CONTROL_TRANSFER_SEQ","COMPLETED_DEVICE_HS_CONTROL_TRANSFER_HS_SEQ ",UVM_LOW)
    endtask
 endclass
