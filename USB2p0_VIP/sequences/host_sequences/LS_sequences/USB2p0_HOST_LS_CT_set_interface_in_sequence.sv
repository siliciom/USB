...
class usb2p0_host_ls_ct_set_interface_in_sequence extends USB2p0_HOST_sequence;
  
	`uvm_object_utils(usb2p0_host_ls_ct_set_interface_in_sequence)
  	USB2p0_sequence_item req;

       function new(string name="usb2p0_host_ls_ct_set_interface_in_sequence");
            super.new(name);
       endfunction

    task body();
           `uvm_info("LS_SET_INTERFACE","ENTERED_INTO_LS_SET_INTERFACE_SEQ ",UVM_LOW)
		...
		`uvm_fatal("RAND_FAIL", "HOST_LS_data_stage_seq_Randomization failed")
          ...
          finish_item(req);
	  req.print(); 
           `uvm_info("LS_SET_INTERFACE","COMPLETED_LS_CT_SET_INTERFACE_SEQUENCE ",UVM_LOW)
