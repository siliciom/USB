...
class usb2p0_host_ls_ct_crc5_error_sequence extends USB2p0_HOST_sequence;
  
	`uvm_object_utils(usb2p0_host_ls_ct_crc5_error_sequence)
  	USB2p0_sequence_item req;

       function new(string name="usb2p0_host_ls_ct_crc5_error_sequence");
            super.new(name);
       endfunction
    ...
    task body();
		...
		`uvm_fatal("RAND_FAIL", "HOST_LS_data_stage_seq_Randomization failed")
          ...
          finish_item(req);
	  req.print(); 
           `uvm_info("LS_CRC5_ERROR","COMPLETED_LS_CRC5_ERROR_SEQ ",UVM_LOW)
