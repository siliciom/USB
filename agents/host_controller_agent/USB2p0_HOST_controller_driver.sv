	       ...
	       `uvm_info("HOST_CONTROLLER_DRIVER", $sformatf("HOST CONTROLLER DRIVER: utmisrp_bvalid=%b",utmi_interface_rx.utmisrp_bvalid),UVM_LOW)
	        if(utmi_interface_rx.utmisrp_bvalid==1) begin     
	         `uvm_info(get_full_name(), $sformatf("HOST_CONTROLLER_DRIVER: utmisrp_bvalid=%b",utmi_interface_rx.utmisrp_bvalid),UVM_LOW)
	          `uvm_info(get_full_name(), $sformatf("--- POWER_CONNECTED_TO_HOST---"),UVM_LOW)
		     ...
		     usb_seq_item.hs_device_detected =1;
              ...
              `uvm_fatal("HOST_DRIVER","---------HOST_IS_NO_CONNECTED_TO_THE_POWER/DEVICE--------")
      ...
      task send_reset();
         ...
         `uvm_info("HOST","ASSERT RESET",UVM_LOW)
           //utmi_interface_rx.utmi_linestate <= 2'b00;
            utmi_interface_tx.utmi_linestate <= 2'b00;
          //device_utmi_interface_rx.utmi_linestate <= 2'b00;
   	 ...
   	 fork begin
      		...
      		wait(device_utmi_interface_tx.utmi_linestate == 2'b01);
                #3us;
	...
	task send_host_chirp_kj();
  	     `uvm_info("HOST_HS","Sending KJ Chirp Sequence",UVM_LOW)
            ...
            repeat (3) begin
                ...
                #1;
                `uvm_info("HOST_HS",$sformatf("HIGH_SPEED_CHRIP_SEQUENCE:utmi_linestate=%b",utmi_interface_tx.utmi_linestate),UVM_LOW)
                seq_done_ev.trigger();
                 #50us;
         ...
         case (usb_speed)
            ...
            case (usb_seq_item.transfer_type)
              ...
              USB2p0_sequence_item::BULK_TRANSFER: begin
                `uvm_warning("USB_PROTO","BULK_TRANSFER_NOT_SUPPORTED_IN_LOW_SPEED")
              end
 
              USB2p0_sequence_item::ISO_CHRONOUS_TRANSFER: begin
                 `uvm_warning("TRANSFER_TYPE","ISOCHRONOUS_TRANSFER_NOT_SUPPORTED_IN_LOW_SPEED")
              end
            endcase
            end
...
///////////////////////////////////////////////////////////////////////////////////////
	 ...
	 `uvm_info("HOST_DRIVER",$sformatf("ENTERED_INTO_DATA_STAGE"),UVM_LOW)
	    ...
	    `uvm_info("HOST_DRIVER",$sformatf("RECEIVED_DATA_STAGE_IN_TOKEN_PKT"),UVM_LOW)
            ...
            ack_done_get_status_ev.wait_trigger();          
             ...
             wait(rx_data_get_status[7:0] == 'hb4) begin
    ...
    endtask
	   ...
	   `uvm_info("HOST_DRIVER",$sformatf("ENTERED_INTO_HANDSHAKE_PKT"),UVM_LOW)
           ...
           wait(utmi_interface_rx.utmi_rxactive && (utmi_interface_rx.utmi_rxvalid || utmi_interface_rx.utmi_rxvalidh)) begin
             ...
             else if (rx_data == 8'he1)begin
               //`uvm_fatal("HOST_DRIVER",$sformatf("STALL_RECEIVED_FROM_DEVICE=%h",rx_data))
	         `uvm_info("HOST_DRIVER",$sformatf("STALL_RECEIVED_FROM_DEVICE=%h",rx_data),UVM_LOW)
             end
           end
         endtask
 ...
 task send_host_handshake(bit [3:0] handshake_pid);
   ...
   bit [7:0]  hs_pid_byte;
   //bit [15:0] hs_pid_word;

   hs_pid_byte = {handshake_pid,~handshake_pid};
   //hs_pid_word = {8'h00, hs_pid_byte};
   ...
   if(!utmi_interface_tx.utmi_word_if) begin
      ...
      utmi_interface_tx.utmi_txvalidh <= 1'b0;
      `uvm_info("HOST_DRIVER",$sformatf("HOST_HANDSHAKE_8BIT txdata=%0h",hs_pid_byte),UVM_LOW)
   end
   else begin
      ...
      `uvm_info("HOST_DRIVER",$sformatf("HOST_HANDSHAKE_16BIT txdata=%0h",hs_pid_byte),UVM_LOW)
   end
   ...
   if(handshake_pid == PID_ACK)
      `uvm_info("HOST_DRIVER",$sformatf("HOST_SENT_ACK = %0h",hs_pid_byte),UVM_LOW)
   else if(handshake_pid == PID_NAK)
      `uvm_info("HOST_DRIVER",$sformatf("HOST_SENT_NAK = %0h",hs_pid_byte),UVM_LOW)
   else if(handshake_pid == PID_STALL)
      `uvm_info("HOST_DRIVER",$sformatf("HOST_SENT_STALL = %0h",hs_pid_byte),UVM_LOW)
endtask
...
///////////////////////////////////////////////////////////////////
     ...
     endtask
      ...
      task receive_get_status();
           ...
           wait(utmi_interface_rx.utmi_rxvalid && utmi_interface_rx.utmi_rxdata[7:0] == 'hb4) begin
                 rx_data_get_status = utmi_interface_rx.utmi_rxdata[7:0]; 
   ...
   //////////////////////////////////////////////////////////////////
     task interrupt_transfer(USB2p0_sequence_item usb_seq_item);
         ...
         `uvm_info("INTERRUPT","IN_TRANSFER",UVM_LOW)
          ...
          `uvm_info("FSM",$sformatf("DATA_STAGE_ENTERED_FROM_DEIVCE_TO_HOST"),UVM_LOW)
            send_to_utmi_tx(usb_seq_item);
             receiving_data_from_device(usb_seq_item);
          end
     endtask
    ...
    task receiving_data_from_device(USB2p0_sequence_item usb_seq_item);
      ...
      // case(usb_seq_item.transfer_type)
      //   USB2p0_sequence_item::INTERRUPT_TRANSFER       : max_payload_size = 64;
      //   USB2p0_sequence_item::BULK_TRANSFER            : max_payload_size = 512;
      //   USB2p0_sequence_item::ISO_CHRONOUS_TRANSFER    : max_payload_size = 1024;
      // endcase
      //////////////////////////////////////////////////////////////////
       case (usb_speed)
             USB_LS: begin
               case (usb_seq_item.transfer_type)
                      USB2p0_sequence_item::INTERRUPT_TRANSFER :
                      max_payload_size = 8;

                      USB2p0_sequence_item::BULK_TRANSFER : begin
                      `uvm_warning("USB","BULK_TRANSFER_NOT_SUPPORTED_IN_LOW_SPEED")
                      return;
                    end

                    USB2p0_sequence_item::ISO_CHRONOUS_TRANSFER : begin
                    `uvm_warning("USB","ISO_TRANSFER_NOT_SUPPORTED_IN_LOW_SPEED")
                     return;
                  end
               endcase
             end
             ...
             USB_FS: begin
               case (usb_seq_item.transfer_type)
                   ...
                   USB2p0_sequence_item::BULK_TRANSFER :
                   max_payload_size = 64;

                   USB2p0_sequence_item::ISO_CHRONOUS_TRANSFER :
          ...
          endcase
        `uvm_info("HOST_DRIVER","WAITING_FOR_DEVICE_DATA", UVM_LOW)

         wait(utmi_interface_rx.utmi_rxvalid);
         @(posedge utmi_interface_rx.utmi_clk);
          pid = utmi_interface_rx.utmi_rxdata[7:0];
           `uvm_info("HOST_DRIVER",$sformatf("PID_RECEIVED = %0h",pid),UVM_LOW)
          ...
          while(utmi_interface_rx.utmi_rxvalid) begin
            ...
            if(utmi_interface_rx.utmi_rxvalid) begin
             ...
             `uvm_info("HOST_DRIVER",$sformatf("PAYLOAD_DATA = %0p",payload_q),UVM_LOW)
            end
          end

         // interrupt_transfer_ev.wait_trigger();
          if(payload_q.size() > max_payload_size) begin
              `uvm_warning("USB_OVERSIZE_ERROR", $sformatf("PAYLOAD_SIZE=%0d EXCEEDED MAX=%0d",payload_q.size(), max_payload_size))
               send_host_handshake(PID_STALL);
               `uvm_info("HOST_DRIVER","STALL_SENT_TO_DEVICE", UVM_LOW)
          end
           else begin
            `uvm_info("HOST_DRIVER",$sformatf("VALID_PAYLOAD_RECEIVED_SIZE=%0d",payload_q.size()),UVM_LOW)
             send_host_handshake(PID_ACK);
            `uvm_info("HOST_DRIVER","ACK_SENT_TO_DEVICE",UVM_LOW)
           end
        endtask
    ...
    //////////////////////////////////////////////////////////////////
      task receive_handshake_packet();

         bit [7:0] rx_pid;
           `uvm_info("HOST_DRIVER","ENTERED_INTO_RECEIVE_HANDSHAKE_PACKET",UVM_LOW)
            wait(utmi_interface_rx.utmi_rxvalid);
             rx_pid = utmi_interface_rx.utmi_rxdata;
              `uvm_info("HOST_DRIVER",$sformatf("RECEIVED_HANDSHAKE_PID = %0h", rx_pid),UVM_LOW)

             if(rx_pid == 8'h2D) begin
               `uvm_info("HOST_DRIVER","ACK_RECEIVED",UVM_LOW)
             end
             else if(rx_pid == 8'hE1) begin
               `uvm_info("HOST_DRIVER","STALL_RECEIVED",UVM_LOW)
             end
    ...
    //////////////////////////////////////////////////////////////////
     ...
     `uvm_info("HOST_DRIVER",$sformatf("OVERSIZED_PAYLOAD_EXPECTING_STALL_FROM_DEVICE = %0d MAX_PKT = %0d SENDING_TO_DEVICE",usb_seq_item.host_payload.size(),max_pkt),UVM_LOW)
       end   
  ...
  endclass
    ...
    /*task receiving_interrupt_data_from_device();
         wait(utmi_interface_rx.utmi_rxvalid && utmi_interface_rx.utmi_rxdata[7:0] == 'hb4) begin
             rx_data_interrupt = utmi_interface_rx.utmi_rxdata[7:0]; 
    ...
    /* task receiving_iso_data_from_device();
         wait(utmi_interface_rx.utmi_rxvalid && utmi_interface_rx.utmi_rxdata[7:0] == 'hb4) begin
             rx_data_isochronous = utmi_interface_rx.utmi_rxdata[7:0]; 
   ...
   /*  task receiving_bulk_data_from_device();
         wait(utmi_interface_rx.utmi_rxvalid && utmi_interface_rx.utmi_rxdata[7:0] == 'hb4) begin
             rx_data_bulk = utmi_interface_rx.utmi_rxdata[7:0]; 
