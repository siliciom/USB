...
class usb2p0_scoreboard extends uvm_scoreboard;
  `uvm_component_utils(usb2p0_scoreboard)
    ...
    function void write_host_tx(USB2p0_sequence_item pkt);
      ...
      if(pkt.tx_valid || pkt.tx_validh) begin
         if(pkt.tx_valid)begin
           host_tx_q.push_back(pkt.utmi_txdata);
           `uvm_info("HOST_TX_SB",$sformatf("8BIT_HOST_TX DATA=%p 8_BIT_QUEUE=%P",pkt.utmi_txdata,host_tx_q),UVM_LOW)
         end
        else if(pkt.tx_validh) begin
          host_tx_q.push_back(pkt.utmi_txdata);
          `uvm_info("HOST_TX_SB",$sformatf("16BIT_HOST_TX DATA=%p 16_BIT_QUEUE=%P VALIDH=%0b ",pkt.utmi_txdata,host_tx_q,pkt.tx_validh),UVM_LOW)
        end
      end
    ...
    function void write_device_rx(USB2p0_sequence_item pkt);
     `uvm_info("DEVICE_RX_SB",$sformatf("DEVICE_RX_DATA"),UVM_LOW)
      pkt.print();
      if(pkt.rx_valid || pkt.rx_validh) begin
         if(pkt.rx_valid)begin
            device_rx_q.push_back(pkt.rx_data);
           `uvm_info("DEVICE_RX_SB",$sformatf("8BIT_DEVICE_RX DATA=%p 8_BIT_QUEUE=%p",pkt.rx_data,device_rx_q),UVM_LOW)
         end
        else if(pkt.rx_validh) begin
           device_rx_q.push_back(pkt.rx_data);
          `uvm_info("DEVICE_RX_SB",$sformatf("16BIT_DEVICE_RX DATA=%p 16_BIT_QUEUE=%p",pkt.rx_data,device_rx_q),UVM_LOW)
        end
      end
      compare_host_to_device();
    endfunction
     ...
     pkt.print();
      if(pkt.tx_valid || pkt.tx_validh) begin
         if(pkt.tx_valid)begin
           ...
           `uvm_info("DEVICE_TX_SB",$sformatf("8BIT_DEVICE_TX DATA=%p 8_BIT_QUEUE=%P",pkt.utmi_txdata,device_tx_q),UVM_LOW)
         end
         else if(pkt.tx_validh) begin
           device_tx_q.push_back(pkt.utmi_txdata);
          `uvm_info("DEVICE_TX_SB",$sformatf("16BIT_DEVICE_TX DATA=%p 16_BIT_QUEUE=%P VALIDH=%0b ",pkt.utmi_txdata,device_tx_q,pkt.tx_validh),UVM_LOW)
         end
      end
      ...
      `uvm_info("HOST_RX_SB",$sformatf("HOST_RX_DATA"),UVM_LOW)
       ...
       if(pkt.rx_valid || pkt.rx_validh) begin
         if(pkt.rx_valid)begin
          ...
          `uvm_info("HOST_RX_SB",$sformatf("8BIT_HOST_RX DATA=%p 8_BIT_QUEUE=%p",pkt.rx_data,host_rx_q),UVM_LOW)
         end
         else if(pkt.rx_validh) begin
           host_rx_q.push_back(pkt.rx_data);
          `uvm_info("HOST_RX_SB",$sformatf("16BIT_HOST_RX DATA=%p 16_BIT_QUEUE=%P VALIDH=%0b ",pkt.utmi_txdata,host_rx_q,pkt.tx_validh),UVM_LOW)
          end
      end
       compare_device_to_host();
    endfunction
    ...
    function void compare_host_to_device();
      ...
      if((host_tx_q.size() > 0) && (device_rx_q.size() > 0)) begin
   ...
   function void compare_device_to_host();
     ...
     bit [DATA_WIDTH-1:0] act_data[$];
    //device_tx_q.delete();
    if((device_tx_q.size() > 0) && (host_rx_q.size() > 0)) begin
